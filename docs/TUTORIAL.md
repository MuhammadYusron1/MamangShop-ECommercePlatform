...
### Step 1.1 — Create the root folder, git repo, and scaffold the client with Vite

We'll build the backend + frontend in one monorepo, but scaffold the **frontend the
idiomatic way with Vite's create tool** rather than hand-writing every config file:
...
# Scaffold the React frontend using Vite's official template.
npm create vite@latest client -- --template react
cd client
npm install                # install the React/Vite deps Vite generated
cd ..
```

This single command creates a complete, working React app in `client/` — `index.html`,
`src/main.jsx`, `src/App.jsx`, `src/index.css`, `public/`, plus ready-made
`package.json`, `vite.config.js`, `.gitignore`, and config files. We then *edit* those
files in **Part 4** instead of creating them by hand.

> 💡 **Why scaffold?** It's fast, uses the official maintained template, zero typos,
> and matches how you'd start any real React project. The downside is a few files
> appear "for free" — so in Part 4 we read each generated file and only then replace
> its contents, which keeps the learning intact.
...
> ℹ️ Once the root workspace exists, run `npm install` **from the repo root** so npm
> hoists everything into a single workspace-aware lockfile. (The earlier installs
> inside `client/` were just to get `npm create vite` working; root install
> re-resolves cleanly. We'll install the backend deps the same root way after Part 2.)

### Step 1.3 — Install the frontend libraries (Redux, Router, Tailwind, axios)

The Vite template only added React + Vite. Add the rest of our client deps from the
`client` folder:
...
### Step 1.4 — Set up `.gitignore` FIRST (security)
...
>
> ℹ️ `npm create vite` also generates a `client/.gitignore` — you can leave it; the
> root `.gitignore` above adds the secret-protection rules that matter.
...
Now the storefront. Because we already scaffolded `client/` with Vite in Step 1.1,
this part mostly **edits generated files** and **creates the app-specific ones**
(slices, pages, components). We'll use Redux Toolkit, React Router, and Tailwind.

### Step 4.1 — What Vite scaffolded for you (read it first)

After Step 1.1–1.3, your `client/` folder was generated by Vite and already has:
`index.html`, `src/main.jsx`, `src/App.jsx`, `src/index.css`, `src/assets/`,
`public/`, `package.json`, `vite.config.js`, `tailwind.config.js`,
`postcss.config.js`. **Read each generated file now** — treat the scaffold itself
as part of the lesson. Then we'll replace/edit their contents below.

### Step 4.2 — Configure Vite (proxy, port), Tailwind (palette + dark mode), PostCSS, index.html

The scaffold created these files with default contents. **Edit** them:

**`client/vite.config.js`** — add the dev proxy to the backend + port 3000:
```js
...
**`client/tailwind.config.js`** — brand palette + manual dark mode (overwrite what
`npx tailwindcss init` produced):
```js
export default {
...
**`client/postcss.config.js`** — `npx tailwindcss init -p` already created this with
tailwind + autoprefixer plugins; confirm it looks like this:
```js
export default { plugins: { tailwindcss: {}, autoprefixer: {} } };
```

**`client/index.html`** — give it a title + the anti-flash dark-mode script
(overwrite the template title):
```html
<!DOCTYPE html>
<html lang="en">
  <head>
...
> ℹ️ If your scaffold's `index.html` differs (newer Vite versions add an svg icon
> link etc.), keep the extra bits — just make sure title + the dark-mode script are
> there.

**`client/src/index.css`** — overwrite the scaffold's style with Tailwind directives +
...
`main.jsx` was generated by Vite — now we **rewrite it** to add Redux + Router
providers. Then **rewrite `App.jsx`** (Vite's default counter demo) into our routing
table:
...
> 🧹 **Clean up scaffold leftovers:** Vite's template adds a demo `src/assets/`
> folder and may import an `App.css` in the default `App.jsx`. Since we rewrite
> `App.jsx` and `index.css`, you can delete `src/assets/` and `src/App.css` (if
> present) so only the files we actually use remain.
